...
class Blockchain {
  constructor({ difficulty = 4, persist = true } = {}) {
    ...
    // Initialize WAL for durable writes
    this.chainWAL = new WAL(DATA_DIR, 'chain.json');
    this.stateWAL = new WAL(DATA_DIR, 'state.json');

    if (this.persist && fs.existsSync(CHAIN_FILE)) {
      try {
        const raw = this.chainWAL.read();
  ...
  getCurrentDifficulty() {
    // adjust difficulty every ADJUST_INTERVAL blocks
    // TARGET_TIME_MS is the desired block time (2 minutes = 120000 ms)
    const ADJUST_INTERVAL = 10;
    const TARGET_TIME_MS = 120000; // target time per block in ms (2 minutes)
  ...
  save() {
    ...
    // perform WAL writes asynchronously (non-blocking event loop)
    (async () => {
      try {
        // Write chain using WAL (atomic)
        await this.chainWAL.writeAtomicAsync(this.chain);
        
        // Write state (balances) using WAL (atomic)
        const stateData = {};
        for (const [addr, bal] of Object.entries(this.balances)) {
          stateData[addr] = bal.toString();
        }
        await this.stateWAL.writeAtomicAsync(stateData);
      } catch (err) {
